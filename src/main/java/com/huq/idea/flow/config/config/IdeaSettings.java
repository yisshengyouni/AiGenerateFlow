...
//            "4.技术注释：右对齐灰色注释框" +
    ...
    public static final String DEFAULT_OPTIMIZE_CODE_PROMPT = "你是一个高级Java开发专家和架构师。请仔细分析提供的Java代码，指出可以优化的点（如性能、可读性、可维护性等），并提供优化后的代码。\n" +
            "请遵循以下规则：\n" +
            "1. 审查代码的规范性、可读性、可维护性和性能。\n" +
            "2. 针对可以优化的部分，给出优化方案和修改建议。\n" +
            "3. 输出优化后的完整代码。\n" +
            "4. 回复使用Markdown格式排版，清晰易读。\n\n" +
            "下面是需要优化的代码：\n%s";
        ...
        public List<PromptConfig> getOptimizePrompts() {
            if (optimizePrompts == null || optimizePrompts.isEmpty()) {
                ...
                if (optimizeCodePrompt != null && !optimizeCodePrompt.isEmpty()) {
                    ...
                    optimizePrompts.add(new PromptConfig("Default", DEFAULT_OPTIMIZE_CODE_PROMPT));
                }
            }
