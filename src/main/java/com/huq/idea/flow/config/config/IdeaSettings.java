...
//            "4.技术注释：右对齐灰色注释框" +
    ...
    public static final String DEFAULT_CALL_CHAIN_ANALYSIS_PROMPT = "你是一位资深的Java架构师和代码审查专家。这是代码调用链的一部分。\n" +
            "你的任务是彻底理解这部分代码的业务逻辑。\n\n" +
            "【行动指南】\n" +
            "1. 如果你认为目前提供的代码片段不足以让你完全理解整个业务流程（例如，里面调用了其他重要的外部接口、数据库操作、或者复杂的Service/Mapper方法，且你现在没有它们的源码），你需要请求查看这些方法的源码。\n" +
            "   - **请求源码格式：** 请严格返回一个 JSON 数组，数组元素为你想要查看的方法的完全限定名（例如：`[\"com.example.service.UserService.getUserById\", \"com.example.mapper.OrderMapper.insert\"]`）。除此之外，不要输出任何多余的解释文字！\n" +
            ...
            "【注意】如果你返回了 JSON 数组，系统会自动去查找源码并追加在下一轮对话中发给你。请明智地决定是否还需要更多代码。\n\n" +
            "【当前提供的代码】\n%s";
