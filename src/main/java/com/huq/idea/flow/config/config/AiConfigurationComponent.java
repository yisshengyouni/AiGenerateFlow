...
public class AiConfigurationComponent {
    ...
    public void init(IdeaSettings.State state) {
        ...
        optimizePromptConfigs = new ArrayList<>();
        if (state.getOptimizePrompts() != null) {
    ...
    private void createPromptConfigPanel() {
        ...
        diagramTypeComboBox = new JComboBox<>(new String[]{"流程图 (Flow Diagram)", "类图 (Class Diagram)", "时序图 (Sequence Diagram)", "状态图 (State Diagram)", "代码解释 (Explain Code)", "代码审查 (Review Code)", "生成单元测试 (Generate Unit Test)", "代码优化 (Optimize Code)"});
