...
public class ReviewCodeAction extends AnAction implements DumbAware {
    ...
    private void showInitialDialog(Project project, CallStack callStack, String collectedCode, String title) {
        ...
        JPanel reviewTab = com.huq.idea.flow.apidoc.ui.CodeAnalysisUIFactory.createAnalysisPanel(
                project,
                collectedCode,
                "代码审查",
                ...
                "审查代码",
                0.7,
                () -> IdeaSettings.getInstance().getState().getReviewPrompts().isEmpty() ? "" : IdeaSettings.getInstance().getState().getReviewPrompts().get(0).getPrompt()
        );
