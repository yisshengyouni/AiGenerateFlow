...
public class OptimizeCodeAction extends AnAction implements DumbAware {
    ...
    public void actionPerformed(AnActionEvent e) {
        ...
        SwingUtilities.invokeLater(() -> {
            CodeAnalysisUIFactory.showAnalysisDialog(
                    project,
                    collectedCode,
                    title,
                    "优化代码",
                    code -> IdeaSettings.getInstance().getState().getOptimizeCodePrompt(),
                    "你是一个高级Java开发专家和重构工程师。请提供优化后的代码。如果包含Markdown代码块符号(如```java)，请去掉，只输出纯代码。",
                    "优化代码"
            );
        });
    }
