...
/**
 * Action to generate UML state diagrams from Java code
 *
 * @author huqiang
 * @since 2024/8/10
 */
public class StateDiagramAction extends AnAction {
    ...
    public void actionPerformed(@NotNull AnActionEvent e) {
        ...
        new Task.Backgroundable(project, "收集类信息", true, PerformInBackgroundOption.ALWAYS_BACKGROUND) {
            @Override
            public void run(@NotNull ProgressIndicator indicator) {
                indicator.setIndeterminate(true);
                indicator.setText("正在收集源码...");

                // 仅收集当前类的代码即可，因为状态通常集中在枚举或单类中
                String collectedCode = ReadAction.compute(() -> {
                    String classCode = currentClass.getText();
                    if (classCode != null && !classCode.isEmpty()) {
                        return "\n\n// " + "=".repeat(80) + "\n" +
                               "// Class: " + currentClass.getQualifiedName() + "\n" +
                               "// " + "=".repeat(80) + "\n\n" +
                               classCode;
                    }
                    return "";
