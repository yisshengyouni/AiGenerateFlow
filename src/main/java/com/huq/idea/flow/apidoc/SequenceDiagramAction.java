 ...
 * @since 2024/8/10
 */
public class SequenceDiagramAction extends AnAction implements DumbAware {
    private static final Logger LOG = Logger.getInstance(SequenceDiagramAction.class);
    ...
    private void showInitialDialog(Project project, CallStack callStack, String collectedCode, String title) {
        JFrame frame = new JFrame("UML时序图: " + title);
...
//        frame.setVisible(true);
    ...
    private JPanel createInitialPlantUmlTab(Project project, JFrame parentFrame, String collectedCode) {
        ...
        JLabel waitingLabel = new JLabel("点击\"生成流程\"按钮生成时序图", SwingConstants.CENTER);
        ...
        generateButton.addActionListener(e -> {
            ...
            if (IdeaSettings.getInstance().getState().getPlantumlPathVal() == null ||
                ...
                LOG.info("PlantUML安装路径未配置，无法渲染时序图");
                Notifications.Bus.notify(new Notification(
                        "com.yt.huq.idea",
                        "PlantUML路径缺失",
            ...
            // 在后台任务中执行AI调用
            new Task.Backgroundable(project, "生成时序图", true, PerformInBackgroundOption.ALWAYS_BACKGROUND) {
                @Override
                public void run(@NotNull ProgressIndicator indicator) {
                    indicator.setText("正在生成PlantUML时序图...");
                    ...
                    config.setSystemMessage("你是一个专业的PlantUML时序图生成专家，擅长分析Java代码并生成高质量的时序图。")
                          .setTemperature(0.7)
                          .setMaxTokens(8000);
                    ...
                    if (flowDiagram != null && !flowDiagram.isEmpty()) {
                        ...
                        SwingUtilities.invokeLater(() -> {
                            String errorMsg = "生成时序图失败，请检查API设置和网络连接";
                            if (response != null && !response.isSuccess() && response.getErrorMessage() != null) {
                                errorMsg = "生成时序图失败: " + response.getErrorMessage();
                            }
        ...
        saveSvgButton.addActionListener(e -> {
            ...
            if (userSelection == JFileChooser.APPROVE_OPTION) {
                ...
                try {
                    ...
                    LOG.error("Failed to render UML diagram as SVG", ex);
                    Notifications.Bus.notify(new Notification(
                            "com.yt.huq.idea",
                            "UML图表",
            ...
            if (userSelection == JFileChooser.APPROVE_OPTION) {
                ...
                try {
                    ...
                    Notifications.Bus.notify(new Notification(
                            "com.yt.huq.idea",
                            "UML图表",
                            "Markdown已导出到 " + fileToSave.getAbsolutePath(),
                            NotificationType.INFORMATION),
                            project);
                } catch (Exception ex) {
                    Notifications.Bus.notify(new Notification(
    ...
    private String getFlowDiagramPrompt() {
        // 从设置中获取时序图提示词
        return IdeaSettings.getInstance().getState().getUmlSequencePrompt();
    }
