...
public class ExplainCodeAction extends AnAction implements DumbAware {
    ...
    public void actionPerformed(AnActionEvent e) {
        ...
        SwingUtilities.invokeLater(() ->
            com.huq.idea.flow.apidoc.ui.CodeAnalysisUIFactory.showInitialDialog(project, collectedCode, "解释代码: " + title, "解释代码", (code, provider, model) -> {
                                java.util.List<com.huq.idea.flow.config.config.IdeaSettings.PromptConfig> prompts = com.huq.idea.flow.config.config.IdeaSettings.getInstance().getState().getExplainPrompts();
                String promptTemplate = prompts != null && !prompts.isEmpty() ? prompts.get(0).getPrompt() : com.huq.idea.flow.config.config.IdeaSettings.getInstance().getState().getExplainCodePrompt();
