...
public class ExplainCodeAction extends AnAction implements DumbAware {
    ...
    private void showInitialDialog(Project project, CallStack callStack, String collectedCode, String title) {
        ...
        JPanel explainTab = com.huq.idea.flow.apidoc.ui.CodeAnalysisUIFactory.createAnalysisPanel(
                project,
                collectedCode,
                "解释代码",
                "点击\"解释代码\"按钮开始分析...",
                ...
                "解释代码",
                0.7,
                () -> IdeaSettings.getInstance().getState().getExplainPrompts().isEmpty() ? "" : IdeaSettings.getInstance().getState().getExplainPrompts().get(0).getPrompt()
        );
