...
public class ExplainCodeAction extends AnAction implements DumbAware {
    ...
    private void showInitialDialog(Project project, CallStack callStack, String collectedCode, String title) {
        CodeAnalysisUIFactory.PromptProvider promptProvider = code -> {
            List<com.huq.idea.flow.config.config.IdeaSettings.PromptConfig> prompts = com.huq.idea.flow.config.config.IdeaSettings.getInstance().getState().getExplainPrompts();
            String promptTemplate = prompts != null && !prompts.isEmpty() ? prompts.get(0).getPrompt() : null;
            if (promptTemplate == null || promptTemplate.isEmpty()) {
                promptTemplate = com.huq.idea.flow.config.config.IdeaSettings.DEFAULT_EXPLAIN_CODE_PROMPT;
            }
            return String.format(promptTemplate, code);
        };

        JPanel explainTab = CodeAnalysisUIFactory.createAnalysisTab(
            project,
            collectedCode,
            "点击\"解释代码\"按钮开始分析...",
            "解释代码",
