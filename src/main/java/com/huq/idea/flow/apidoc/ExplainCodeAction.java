    ...
    private JPanel createExplainTab(Project project, String title, String collectedCode) {
        JPanel panel = CodeAnalysisUIFactory.createAnalysisPanel(
                ...
                code -> {
                    ...
                    String promptTemplate = IdeaSettings.DEFAULT_EXPLAIN_CODE_PROMPT;
                    if (state.getExplainPrompts() != null && !state.getExplainPrompts().isEmpty()) {
                        promptTemplate = state.getExplainPrompts().get(0).getPrompt();
                    }
                    return String.format(promptTemplate, code);
                },
                "你是一个高级Java开发专家和架构师。请提供详细、清晰的代码解释。",
                "代码解释"
        );
        return panel;
    }
