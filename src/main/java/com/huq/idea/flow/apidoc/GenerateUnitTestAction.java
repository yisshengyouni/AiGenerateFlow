...
public class GenerateUnitTestAction extends AnAction implements DumbAware {
    ...
    public void actionPerformed(AnActionEvent e) {
        ...
        SwingUtilities.invokeLater(() ->
            com.huq.idea.flow.apidoc.ui.CodeAnalysisUIFactory.showInitialDialog(project, collectedCode, "单元测试: " + title, "生成测试代码", (code, provider, model) -> {
                                java.util.List<com.huq.idea.flow.config.config.IdeaSettings.PromptConfig> prompts = com.huq.idea.flow.config.config.IdeaSettings.getInstance().getState().getTestPrompts();
                String promptTemplate = prompts != null && !prompts.isEmpty() ? prompts.get(0).getPrompt() : com.huq.idea.flow.config.config.IdeaSettings.getInstance().getState().getGenerateTestPrompt();
                ...
                config.setSystemMessage("你是一个高级Java开发专家和测试工程师。请提供高质量、可以直接运行的JUnit 5单元测试代码。")
                      .setTemperature(0.7)
                      .setMaxTokens(8000);
