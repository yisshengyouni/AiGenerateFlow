    ...
    private JPanel createGenerateTestTab(Project project, String title, String collectedCode) {
        JPanel panel = CodeAnalysisUIFactory.createAnalysisPanel(
                ...
                code -> {
                    ...
                    String promptTemplate = IdeaSettings.DEFAULT_GENERATE_TEST_PROMPT;
                    if (state.getTestPrompts() != null && !state.getTestPrompts().isEmpty()) {
                        promptTemplate = state.getTestPrompts().get(0).getPrompt();
                    }
